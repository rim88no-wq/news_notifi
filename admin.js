...
// ─── Login — stays on Railway (password verified server-side) ─────────────────
...
// ─── Subscriber count — reads Firestore directly ──────────────────────────────
async function loadSubscriberCount() {
  try {
    ...
    const stat = document.getElementById('stat-subs');
    if (stat) stat.textContent = count;
  } catch { /* silent */ }
}

// ─── Send notification — stays on Railway (needs VAPID private key) ───────────
...
// ─── Notification history — reads Firestore directly ─────────────────────────
async function loadHistory() {
  ...
  try {
    ...
    if (statDel)    statDel.textContent    = notifications.reduce((s, n) => s + (n.sentCount || 0), 0);
    if (statLatest && notifications.length > 0) statLatest.textContent = relativeTime(notifications[0].createdAt);

    if (notifications.length === 0) { emptyEl.style.display = ''; return; }
...
function buildHistoryItem(notif) {
  ...
  `;
  item.querySelector('.btn-delete').addEventListener('click', () => deleteNotification(notif.id));
  return item;
}

// ─── Delete notification — writes Firestore directly ─────────────────────────
async function deleteNotification(id) {
  ...
  try {
    ...
    if (el) {
      ...
      setTimeout(() => {
        el.remove();
        if (!document.querySelector('.history-item')) document.getElementById('history-empty').style.display = '';
      }, 260);
    }
  } catch {
...
function switchSection(name) {
  const titles = { send: 'Send Notification', history: 'Notification History', stats: 'Statistics', news: 'Manage News' };
  document.querySelectorAll('.nav-link').forEach((a) => a.classList.toggle('active', a.dataset.section === name));
  document.querySelectorAll('.admin-section').forEach((sec) => (sec.style.display = sec.id === `section-${name}` ? '' : 'none'));
  const h = document.getElementById('section-heading');
  if (h) h.textContent = titles[name] || name;
  if (name === 'history' || name === 'stats') { loadHistory(); loadSubscriberCount(); }
  if (name === 'stats') loadArticleCount();
  if (name === 'news')  loadAdminArticles();
}

function clearForm() {
  ['notif-title', 'notif-body'].forEach((id) => { const el = document.getElementById(id); if (el) el.value = ''; });
  ...
  ['send-success', 'send-error'].forEach((id) => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ARTICLES — all operations go directly to Firestore
// ═══════════════════════════════════════════════════════════════════════════════
...
async function loadAdminArticles() {
  ...
  try {
    ...
    if (!articles.length) { if (emptyEl) emptyEl.style.display = ''; return; }
    listEl.innerHTML = articles.map(buildAdminArticleRow).join('');
    listEl.querySelectorAll('.btn-edit-article').forEach((btn) => btn.addEventListener('click', () => editArticle(btn.dataset.id)));
    listEl.querySelectorAll('.btn-delete-article').forEach((btn) => btn.addEventListener('click', () => deleteArticle(btn.dataset.id)));
  } catch {
...
function buildAdminArticleRow(a) {
  const COLORS = {
    politics: 'linear-gradient(135deg,#92400e,#f59e0b)', tech: 'linear-gradient(135deg,#1e40af,#3b82f6)',
    sports:   'linear-gradient(135deg,#065f46,#10b981)', economy: 'linear-gradient(135deg,#9d174d,#ec4899)',
    science:  'linear-gradient(135deg,#5b21b6,#8b5cf6)', health: 'linear-gradient(135deg,#14532d,#22c55e)',
    world:    'linear-gradient(135deg,#0c4a6e,#0ea5e9)',
  };
  const bg = a.imageColor || COLORS[(a.category || '').toLowerCase()] || COLORS.world;
  return `
    <div class="article-row" data-id="${esc(a.id)}">
      <div class="article-row-thumb" style="background:${bg}" aria-hidden="true">${esc(a.imageEmoji || '📰')}</div>
      ...
      <div class="article-row-actions">
        <button class="btn-edit-article btn-icon" data-id="${esc(a.id)}" title="Edit" aria-label="Edit">✏️</button>
        <button class="btn-delete-article btn-del"  data-id="${esc(a.id)}" title="Delete" aria-label="Delete">🗑</button>
      </div>
    </div>`;
}
...
async function handleArticleSave(e) {
  ...
  try {
    if (editId) {
      await db.collection('articles').doc(editId).update({ ...payload, updatedAt: new Date().toISOString() });
    } else {
      const id  = crypto.randomUUID();
      const now = new Date().toISOString();
      await db.collection('articles').doc(id).set({ id, ...payload, createdAt: now, updatedAt: now });
    }
    if (successEl) {
...
async function deleteArticle(id) {
  ...
  try {
    ...
    if (row) {
      ...
      setTimeout(() => {
        row.remove();
        if (!document.querySelector('.article-row')) document.getElementById('articles-empty').style.display = '';
      }, 260);
    }
  } catch {
...
function clearArticleForm() {
  ...
  ['art-title', 'art-excerpt', 'art-body', 'art-color'].forEach((id) => { const el = document.getElementById(id); if (el) el.value = ''; });
  const cat = document.getElementById('art-category');     if (cat)     cat.value      = '';
  const auth = document.getElementById('art-author');      if (auth)    auth.value     = 'Staff Reporter';
  const emoji = document.getElementById('art-emoji');      if (emoji)   emoji.value    = '📰';
  const url = document.getElementById('art-url');          if (url)     url.value      = '/';
  const feat = document.getElementById('art-featured');    if (feat)    feat.checked   = false;
  const formTitle = document.getElementById('article-form-title');
  if (formTitle) formTitle.textContent = 'Add New Article';
  const saveBtn = document.getElementById('article-save-btn');
  if (saveBtn) saveBtn.innerHTML = '<span aria-hidden="true">💾</span> Save Article';
  ['article-success', 'article-error'].forEach((id) => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
}
